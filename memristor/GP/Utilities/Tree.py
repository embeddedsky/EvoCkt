...
class Tree:
    ...
    def generate_random_function(self) -> Node:
        ...
        if rand_label[0]=='R':
            ...
            if value > math.log(self.R_Set[0][2][1] / 10000.0, 10):
                value = math.log(self.R_Set[0][2][1] / 10000.0, 10)
            if value < math.log(self.R_Set[0][2][0] / 10000.0, 10):
                value = math.log(self.R_Set[0][2][0] / 10000.0, 10)
    ...
    def populate_random_tree(self, method: str):
        ...
        if self.depth < 2:
            ...
            if method is "full":
                ...
                while self.terminal_node_num<=5 :
                    self.terminal_node_num=0
                    self.root = self.generate_random_root_function()
    ...
    def check_terminal(self):
        ...
        if self.terminal_count_set[1] == 0:
            ...
            if len(not_must_pos)==0:
                ...
                print("")
            rand_index=random.randrange(0,len(not_must_pos))
            self.__temp_terminals[not_must_pos[rand_index]]=1
        self.current_terminal_node = 0
        self.terminal_value(self.root)
        self.init_port_list(self.root)
        self.left_left(self.root)
        ...
        if parent is not None:
            if parent.type=='T':
                if operate =='CUT':
                    ...
                    raise Exception('what the operation？')
            else:
                for i in range(parent.max_num_of_children):
    ...
    def create_terminal(self):
        ...
        for i in range(0,self.terminal_node_num):
            random_value=self.terminal_set[random.randrange(1,len(self.terminal_set))]
        ...
        if parent_node is not None:
            if parent_node.type is "F":
                if parent_node.label=="R":
                    ...
                    if parent_node.value_tree.root.value>math.log(self.R_Set[0][2][1]/10000.0,10):
                        parent_node.value_tree.root.value = math.log(self.R_Set[0][2][1] / 10000.0, 10)
                    if  parent_node.value_tree.root.value<math.log(self.R_Set[0][2][0]/10000.0,10):
                        parent_node.value_tree.root.value = math.log(self.R_Set[0][2][0] / 10000.0, 10)
