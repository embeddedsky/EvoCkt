...
class GP:
    ...
    def evolve(self,iterate):
        ...
        mutate_num = 0
        if iterate<1000:
            mutate_num=int(0.5*self.population_size)
        else:
            mutate_num=int(0.2*self.population_size)

        for i in range(self.population_size-mutate_num,self.population_size):
            self.mutation_algorithms = ["another"]
            ...
            while temp_new.terminal_node_num <=5:
                temp_new = deepcopy(new_population[i])
                temp_new.update_tree()
                temp_new=self.mutate(temp_new)
                temp_new.update_tree()
            ...
            pool_parameter = (new_population[i], file_name_add[i])
            cal_fitness_main(pool_parameter)


        self.population=deepcopy(new_population)

        self.mutation_algorithms = ["delete","add"]
        for i in range(start_index, self.population_size):
            ...
            new_population[i] = self.__cross_over(temp_1,temp_2)
            new_population[i].update_tree()

            new_population[i].terminal_node_num = 0
            new_population[i].count_terminal_node(new_population[i].root)

            while new_population[i].terminal_node_num <= 5:
                temp_1 = deepcopy(parent_1)
                temp_2 = deepcopy(parent_2)
            ...
            new_population[i].update_tree()
            new_population[i].adjust_depth_terminal()
            new_population[i].update_tree()
            ...
            parent_1, parent_2 = self.__tournament_selection()
            temp_1 = deepcopy(parent_1)
            temp_2 = deepcopy(new_population[i])
            ...
            if random.random() < self.value_cross_rate:
                ...
                new_population[i] = deepcopy(temp_2)
        ...
        for i in range(0, self.population_size):
            pool_parameter.append((new_population[i], file_name_add[i]))
        for i in range(0, self.population_size):
    ...
    def __value_cross_over(self, parent_1: SubTree, parent_2: SubTree) -> SubTree:
        ...
        if (parent_2.get_random_node() == None):
            return
        temp_node = parent_2.get_random_node()
    ...
    def __mutate_add(self, parent: Tree):
        rand_node_index = random.randrange(0,int(parent.number_of_nodes_in_tree))
        temp_node: Node = parent.get_node(rand_node_index)
        while temp_node.type is "T" :
            rand_node_index = random.randrange(0, parent.number_of_nodes_in_tree)
            temp_node: Node = parent.get_node(rand_node_index)
