...
class Tree:
    ...
    def generate_random_function(self) -> Node:
        ...
        if rand_label[0]=='R':
            ...
            if value < rand_label[2][0]:
                value = rand_label[2][0]
            if value > rand_label[2][1]:
                value = rand_label[2][1]
    ...
    def populate_random_tree(self, method: str):
        ...
        if self.depth < 2:
            ...
            if method is "full":
                ...
                while self.terminal_node_num<=3 :
                    self.terminal_node_num=0
                    self.root = self.generate_random_root_function()
        ...
        if parent_node is not None:
            if parent_node.type is "F":
                if parent_node.label=="R":
                    ...
                    if parent_node.value<self.R_Set[0][2][0]:
                        parent_node.value=self.R_Set[0][2][0]
                    if parent_node.value>self.R_Set[0][2][1]:
                        parent_node.value=self.R_Set[0][2][1]
