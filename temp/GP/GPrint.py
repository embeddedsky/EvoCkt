...
def print_file(filename,instruction_set,index):
    with open(filename, "w+") as f:
        ...
        fixed_content = ['guess generator', '',
                         ...
                         '*胚胎电路', '*****input 0 2 3 5***********', 'Vs 4001 0 -5', 'Rs 4001 5 1k','Vc 1001 0 15','Rc 1001 2 1k','Rl 0 3 10k','vdd 888 0 2.5',
                         '*******generate circuit*****************']
        ...
        other_content = ['.save time', '.save v(3)',
                         '*control output', '.control',
                         'run', 'op', 'dc vdd 2 3 0.1','dc temp 0 100 5',
...
def cal_fitness(spicepath,filename,targetpath,tem_targetpath,circuit_area,index):
    ...
    if np.where(arrs!=0)[0].shape[0]==0:
        ...
        #output error
        error_a = np.sum(pow(abs(v_out - v_ref),2))
        error_b=0.0
